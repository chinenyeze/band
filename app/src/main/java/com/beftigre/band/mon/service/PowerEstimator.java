...
package com.beftigre.band.mon.service;

import com.beftigre.band.Band;
import com.beftigre.band.mon.UMLoggerService;
import com.beftigre.band.mon.components.PowerComponent;
import com.beftigre.band.mon.components.OLED;
import com.beftigre.band.mon.phone.PhoneSelector;
import com.beftigre.band.mon.phone.PhoneConstants;
import com.beftigre.band.mon.phone.PowerFunction;
import com.beftigre.band.mon.util.BatteryStats;
import com.beftigre.band.mon.util.Counter;
import com.beftigre.band.mon.util.HistoryBuffer;
import com.beftigre.band.mon.util.NotificationService;
import com.beftigre.band.mon.util.SystemInfo;
...
public class PowerEstimator implements Runnable {
    ...
    for(long iter = -1; !Thread.interrupted(); ) {
      ...
      synchronized(fileWriteLock) {
        if(logStream != null) try {
          ...
          logStream.write("begin " + iter + " " + System.currentTimeMillis() + "\n");
  ...
  private void saveToLog(){
    File writeFile = new File(
            Environment.getExternalStorageDirectory(), "PowerLog_" +
            Band.getFilename() + ".log");
    try {
      InflaterInputStream logIn = new InflaterInputStream(
              context.openFileInput("PowerLog.log"));
      ...
      logIn.close();
      logOut.close();
      Log.i("Band", "Wrote log to " + writeFile.getAbsolutePath());
      return;
    ...
    Log.i("Band", "Failed to write log to sdcard");
  }
